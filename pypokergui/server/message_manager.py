...
def broadcast_config_update(game_manager, sockets):
    for soc in sockets:
        try:
            soc.write_message(_gen_config_update_message(game_manager, soc.uuid))
        except:
            logging.error("Error sending message", exc_info=True)

def _gen_config_update_message(game_manager, uuid):
...
def broadcast_start_game(game_manager, sockets):
    # broadcast message to browser bia sockets
    for soc in sockets:
        try:
            soc.write_message(_gen_start_game_message(game_manager, soc.uuid))
        except:
            logging.error("Error sending message", exc_info=True)
...
def _gen_start_game_message(game_manager, uuid):
    registered = game_manager.get_human_player_info(uuid)
    html_str = self.render_string(
...
def broadcast_update_game(game_manager, sockets, update_interval=5):
    for destination, update in game_manager.latest_messages:
        for uuid in _parse_destination(destination, game_manager, sockets):
            if len(str(uuid)) <= 2:
                ...
                _broadcast_message_to_ai(ai_player, message)
            else:
