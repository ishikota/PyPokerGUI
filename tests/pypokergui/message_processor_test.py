...
class MessageProcessorTest(BaseUnitTest):

    def test_start_game(self):
        uuid_list = ["hoge", "fuga", "boo"]
        name_list = ["HOGE", "FUGA", "BOO"]
        players_info = MP.gen_players_info(uuid_list, name_list)
        game_config = MP.gen_game_config(5, 100, 10, 1)

        mp = MP.MessageProcessor()
        original_msgs = mp.start_game(players_info, game_config)
    ...
    def test_update_game(self):
        ...
        mp = MP.MessageProcessor()
        mp.start_game(players_info, game_config)
        original_msgs = mp.update_game("call", 10)
        hoge_msg, fuga_msg, boo_msg = [_simplify_messages(msgs) for msgs in _classify_messages_by_destination(original_msgs, uuid_list)]
    ...
    def test_update_game_when_round_finished(self):
        ...
        mp = MP.MessageProcessor()
        mp.start_game(players_info, game_config)
        mp.update_game("fold", 0)
        original_msgs = mp.update_game("fold", 0)
        hoge_msg, fuga_msg, boo_msg = [_simplify_messages(msgs) for msgs in _classify_messages_by_destination(original_msgs, uuid_list)]
    ...
    def test_update_game_when_game_finished(self):
        ...
        mp.current_state['table'].deck = Deck(cheat=True, cheat_card_ids=range(6, 11))
        for idx, player in enumerate(mp.current_state['table'].seats.players):
            player.hole_card = [Card.from_id(idx*2), Card.from_id(idx*2+1)]

        mp.update_game("raise", 99)
        mp.update_game("call", 99)
        original_msgs = mp.update_game("call", 99)
        hoge_msg, fuga_msg, boo_msg = [_simplify_messages(msgs) for msgs in _classify_messages_by_destination(original_msgs, uuid_list)]
    ...
    def test_gen_game_config(self):
        config = MP.gen_game_config(5, 100, 10, 1)
    ...
    def test_gen_game_config_with_blind_structure(self):
        config = MP.gen_game_config(5, 100, 10, 1, blind_structure)
        self.eq(1, config['blind_structure'][1]['ante'])
        self.eq(5, config['blind_structure'][2]['ante'])

    def test_get_forced_bet_amount(self):
        self.eq((10, 1), MP._get_forced_bet_amount(1, blind_structure))
        self.eq((20, 5), MP._get_forced_bet_amount(2, blind_structure))
        self.eq((30, 10), MP._get_forced_bet_amount(3, blind_structure))
        self.eq((30, 10), MP._get_forced_bet_amount(4, blind_structure))
        self.eq((50, 20), MP._get_forced_bet_amount(5, blind_structure))
